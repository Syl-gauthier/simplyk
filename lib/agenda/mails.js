...
module.exports = function(agenda) {
  ...
  agenda.define('sendDayBeforeEmail', function(job) {
    ...
    emailer.sendOneDayReminderEmail({
      ...
      customMessage: [job.attrs.data.firstname + ', tu es en forme pour demain ?', 'Si ce n\'est pas déjà fait, n\'oublie pas de contacter ' + job.attrs.data.org_name + ' afin que vous vous retrouviez demain, ' + job.attrs.data.start_date + ', à ' + job.attrs.data.address],
  ...
  agenda.define('sendOneWeekBeforeEmail', function(job) {
    ...
    if (job.attrs.data.phone) {
      sms.sendNexmoSMS({
        phone: job.attrs.data.phone,
        message: 'Salut ' + job.attrs.data.firstname + ', c\'est Simplyk. :) N\'oublie pas que dans quelques jours, tu as du bénévolat avec ' + job.attrs.data.org_name + '. Ce sera ' + job.attrs.data.start_date + ', à ' + job.attrs.data.address
      })
    }
  });
  ...
  agenda.define('longTermExpirationEmail', function(job) {
    ...
    emailer.sendLongTermExpirationEmail({
      ...
      customMessage: ['L\'engagement ' + job.attrs.data.lt_name + ' de ' + job.attrs.data.email + ' vient d\'arriver à sa date d\'expiration et les inscriptions de bénévoles ne sont donc plus possibles.', 'Si vous voulez que les inscriptions restent ouvertes, allez sur la page de l\'engagement pour en modifier la date d\'expiration !'],
      lt_name: job.attrs.data.lt_name
    });
  });
