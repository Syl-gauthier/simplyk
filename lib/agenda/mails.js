  ...
  agenda.define('sendDayBeforeEmail', function(job) {
    ...
    if (job.attrs.data.phone) {
      sms.sendNexmoSMS({
        ...
        message: 'Salut ' + job.attrs.data.firstname + ', c\'est Simplyk. :) N\'oublie pas que ' + job.attrs.data.org_name + ' t\'attend demain, ' + job.attrs.data.start_date + ', à ' + job.attrs.data.address
      })
    }
  });
