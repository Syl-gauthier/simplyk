...
function refreshPreferences(volunteer, callback) {
			...
			vol.events.map(function(ev, i) {
				...
				Organism.findOne({
					...
					if (err) {
						...
						console.log(vol.email + ': ' + i + '/' + ev_number);
						if (i == (ev_number - 1)) {
		...
		p2.then(function() {
			...
			Volunteer.findOneAndUpdate({
				...
				if (err) {
					...
					console.log(vol_i + '/' + vol_number + ': ' + vol.email + ' (' + JSON.stringify(new_preferences) + ')');
					if (vol_i == (vol_number - 1)) {
						return callback(err, vol);
					}
				}
			})
		});
