...
const removeLongterm = function(lt_id, callback) {
	//Remove events from volunteer C
	Volunteer.update({
		...
		if (err) {
			...
			console.log('Longterms from volunteer removed ! Response : ' + JSON.stringify(response));
			//Check if we delete only the activity (if other_activities) or the entire event
			Organism.update({
				'long_terms._id': lt_id
			}, {
				...
				if (err) {
					...
					console.log('Longterms from organism removed ! Response : ' + JSON.stringify(response));
					OrgTodo.remove({
						'lt_id': lt_id
					}, function(err, response) {
						if (err) {
